...
Vamos criar as classes `Thread1` e `Thread2`. As threads de execução criadas por instâncias 
destas classes calculam, respetivamente, as duas subexpressões da <<figure-tasks_in_threads>>(b) e 
guardam os valores calculados.
...
. [[sharedThreadVariableExercicio]] Consider the following code snippet.
+
[source,java]
----
x = 13;
